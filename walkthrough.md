...
We fixed three critical issues reported in production logs:
...
4. **FutureWarning & Attribute Error on SVM:**
   - Replaced deprecated `SVC(probability=True)` with the recommended `CalibratedClassifierCV(SVC(), ensemble=False)` in [classifier.py](file:///c:/work/CyberBullying-Detector/api/app/ml_engine/classifier.py) and [train_model.py](file:///c:/work/CyberBullying-Detector/api/scripts/train_model.py). This resolves the `FutureWarning` in scikit-learn 1.9+ and prevents internal attribute issues (`_effective_probability` AttributeError) when loading models serialized across different scikit-learn versions.
